...
export function attemptToSetLatestForRuntimes(runtimes: Runtime[]): Runtime[] {
	...
	for (let i = 0; i < runtimes.length; i++) {
		const isValidRuntimeLanguage =
			languageVersions.findIndex((r) => r.language === runtimes[i].language) ===
			-1;
		if (isValidRuntimeLanguage) {
			languageVersions.push({
				language: runtimes[i].language,
				versions: [{ index: i, version: runtimes[i].version }],
			});
			continue;
		}
		...
		languageVersions[
			languageVersions.findIndex((r) => r.language === runtimes[i].language)
		].versions.push({ index: i, version: runtimes[i].version });
	}
	...
	for await (const packageDir of packagesDir) {
		if (packageDir.isDirectory()) {
			...
			const runtime = new Runtime(
				...
				Object.fromEntries(
					configObject.environment.map((o: string) => o.split("=")),
				),
