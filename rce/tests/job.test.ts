...
test("should give a default value for timeouts and memoryLimit", () => {
	...
	expect(
		job.compileTimeout,
		`Default value for compileTimeout must be 5_000, instead got ${job.compileTimeout}`,
	).toStrictEqual(10_000);

	expect(
		job.runTimeout,
		`Default value for runTimeout must be 10_000, instead got ${job.runTimeout}`,
	).toStrictEqual(10_000);

	expect(
		job.memoryLimit,
		`Default value for memoryLimit must be 512MB, instead got ${job.memoryLimit}`,
	).toStrictEqual(512 * 1024 * 1024);
});
...
test("should not use default value for timeouts and memoryLimit", () => {
	...
	expect(
		job.compileTimeout,
		`Value for compileTimeout must be 10_000, instead got ${job.compileTimeout}`,
	).toStrictEqual(10_000);

	expect(
		job.runTimeout,
		`Value for runTimeout must be 25_000, instead got ${job.runTimeout}`,
	).toStrictEqual(25_000);

	expect(
		job.memoryLimit === 256 * 1024 * 1024,
		`Value for memoryLimit must be 256MB, instead got ${job.memoryLimit}`,
	);
});
...
test.sequential("should be able to run a file - NodeJS", async (t) => {
	...
	expect(
		result.exitCode,
		`Run result didn't exit with 0, instead it exited with ${result.exitCode} and message ${result.output}`,
	).toStrictEqual(0);

	expect(
		result.output.trim(),
		`File output must be "Hello world~", instead of "${result.output}"`,
	).toStrictEqual("Hello world~");

	expect(
		result.stderr.trim(),
		"File stderr assert must be empty",
	).toStrictEqual("");

	expect(
		result.stdout.trim(),
		`File stdout must be "Hello world~", instead of "${result.stdout}"`,
	).toStrictEqual("Hello world~");
});
...
test.sequential("should be able to compile and run a file - C", async (t) => {
	...
	expect(
		...
		`Compile result didn't exit with 0, instead it exited with ${compileResult.exitCode} and message ${compileResult.output}`,
	).toStrictEqual(0);
	...
	expect(
		runResult.exitCode,
		`Run result didn't exit with 0, instead it exited with ${runResult.exitCode} and message ${runResult.output}`,
	).toStrictEqual(0);

	expect(
		runResult.output.trim(),
		`File output must be "Hello World~", instead of "${runResult.output}"`,
	).toStrictEqual("Hello World~");

	expect(
		runResult.stderr.trim(),
		`File stderr assert must be empty, instead of ${runResult.stderr}`,
	).toStrictEqual("");

	expect(
		runResult.stdout.trim(),
		`File stdout must be "Hello World~", instead of "${runResult.stdout}"`,
	).toStrictEqual("Hello World~");
});

test.sequential(
	"should be able to compile and run a file - Python",
	async (t) => {
...
until i == 8`,
	...
	expect(
		runResult.output.trim().split("\n").join(" "),
		`File output must be "1 7 10 13 19 23 28 31", instead of "${runResult.output}"`,
	).toStrictEqual("1 7 10 13 19 23 28 31");
	...
	expect(
		runResult.stdout.trim().split("\n").join(" "),
		`File stdout must be "1 7 10 13 19 23 28 31", instead of "${runResult.stdout}"`,
	).toStrictEqual("1 7 10 13 19 23 28 31");
});

test.sequential(
	"should be able to execute Happy Numbers - Python",
	async (t) => {
		if (process.env?.LANGUAGE_PYTHON !== "true") {
			t.skip();
			return;
