...
import {
	attemptToSetLatestForRuntimes,
	parseSemver,
} from "../src/runtime/acquire.js";
...
test("should be able to parse semver", () => {
	expect(parseSemver(["3"])).toMatchObject({
		major: 3,
		minor: 0,
		patch: 0,
		edition: "latest",
	});
	expect(parseSemver(["16", "15", "8", "rc-8"])).toMatchObject({
		major: 16,
		minor: 15,
		patch: 8,
		edition: "rc",
	});
	expect(parseSemver(["lorem", "ipsum", "dolor", "sit", "amet"])).toMatchObject(
		{ major: 0, minor: 0, patch: 0, edition: "nightly" },
	);
});
